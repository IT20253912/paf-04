...
function Media() {
  ...
  const defaultImageLink =
    "https://steelsupplements.com/cdn/shop/articles/shutterstock_420286117_1000x.jpg?v=1639391967";
  ...
  const deletePost = async (postId) => {
    try {
      const response = await fetch(
        `http://localhost:5005/api/v1/feed/delete/${postId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const updatePost = async () => {
    try {
      const response = await axios.put(
        `http://localhost:5005/api/v1/feed/edit/${postIdToUpdate}`,
        {
          title: updatedTitle,
